...
##################################### Begining of non-local field function + intermediaire #####################################
...
def intermediaire(T,U):
    ...
    M = np.vstack((np.hstack((K@C,K@D@F)), np.hstack((J@E@C,J@F))))
...
def field(struct, beam, window):
    ...
    def values_nl(k = "int", wavelength = "float") :
        return struct.materials[Type[k]].get_values_nl(wavelength)

    # Wavelength in vacuum.
    lam = beam.wavelength
    # Computation of all the permittivities/permeabilities
    Type = struct.layer_type
    Epsilon_mat, Mu_mat = struct.polarizability(lam)
    ...
    Beta = []
    for i in range(g + 1):
        if struct.materials[Type[i]].beta == 0:
            Beta.append(0)
        else : 
            Beta.append(struct.materials[Type[i]].get_values_nl(lam)[3])

    for nm in np.arange(2 * nmod + 1):
        T = []
        T.append(np.array([[0, 1], [1, 0]]))
        # INITIALISATION DE LA MATRICE DE SCATTERING
        alpha = np.sqrt(Epsilon[0]) * k0 * np.sin(theta) + 2 * np.pi * (nm - nmod)
        gamma = np.array(np.sqrt([Epsilon[i] * k0 ** 2 - alpha ** 2 for i in range(g + 1)]), dtype = complex)
        ...
        if np.real(Epsilon[Type[g]]) < 0 and np.real(
                Mu[Type[g]]) < 0 and np.real(np.sqrt(Epsilon[Type[g]] * k0 ** 2 - alpha ** 2)) != 0:
        ...
        for k in range(g) :
            ...
            # DEFINITION DE KL ET OMEGA POUR LE NON LOCALE / MATRICE DE COUCHE NON LOCALE
            else : # if Beta[k] != 0 :
                Kl[k] = np.sqrt(alpha**2 + (values_nl(k, lam)[2] / Beta[k])**2 * (1 / values_nl(k, lam)[1] + 1 / (1 + values_nl(k, lam)[0]) ))
                omega[k] = (alpha**2 / Kl[k]) * (1 / Epsilon[k] - 1 / (1 + values_nl(k, lam)[0]))
                t = np.exp(1j * gamma[k] * thickness[k])
                l = np.exp(- Kl[k] * thickness[k])
                T.append(np.array([[0, 0, t, 0], [0, 0, 0, l], [t, 0, 0, 0], [0, l, 0, 0]], dtype = complex))


            # local local interface 
            if Beta[k] == 0 and Beta[k + 1] == 0 :
                T.append(np.array([[b1 - b2, 2 * b2], [2 * b1, b2 - b1]] / (b1 + b2), dtype = complex))

            # local non-local interface
            elif Beta[k] == 0 and Beta[k + 1] != 0  :
                Kl[k + 1] = np.sqrt(alpha**2 + (values_nl(k + 1, lam)[2] / Beta[k + 1])**2 * (1 / values_nl(k + 1, lam)[1] + 1 / (1 + values_nl(k + 1, lam)[0]) ))
                omega[k + 1] = (alpha**2 / Kl[k + 1]) * (1 / Epsilon[k + 1] - 1 / (1 + values_nl(k + 1, lam)[0]))
                T.append(np.array([[b1 - b2 + 1j * omega[k + 1], 2 * b2, 2], [2 * b1, b2 - b1 + 1j * omega[k + 1], 2], [2 * 1j * omega[k + 1] * b1, 2 * 1j * omega[k + 1] * b2, b1 + b2 + 1j * omega[k + 1]]] / (b1 + b2 - 1j * omega[k + 1]), dtype = complex))
            
            # non-local local interface
            
            elif Beta[k] != 0 and Beta[k + 1] == 0  :
                T.append([[b1 - b2 + 1j * omega[k], -2, 2 * b2], [-2 * 1j * omega[k] *  b1, b1 + b2 + 1j * omega[k], -2  * 1j * omega[k] * b2], [2 * b1, -2, b2 - b1 + 1j * omega[k]]] / (b1 + b2 - 1j * omega[k]))
            
            # non-local non-local interface
            else : # Beta[k] != 0 and Beta[k + 1] != 0  :
        ...
        # Initialization for the Scattering matrix from 0 to n-layer and from the last layer to n-layer
        H = [T[2 * g]]
        A = [T[0]]

        for k in range(len(T)-2):
            A.append(cascade(A[k], T[k + 1]))
            H.append(cascade(T[len(T) - k - 2], H[k]))

        I = []
        for k in range(len(T) - 1):
            I.append(intermediaire(A[k],H[len(T) - k - 2]))
        I.append(np.array([[0, 0], [0, 0]]))
        ...
        for k in range(g + 1):
            for m in range(int(ny[k])):
                ...
                if np.shape(I[2*k+1]) == (2,2):
                    E[t, 0] = I[2 * k][0, 0] * np.exp(1j * gamma[k] * h) + I[2 * k+1][1, 0] * np.exp(1j * gamma[k] * (thickness[k] - h))
                else : 
                    E[t, 0] = I[2 * k][0, 0] * np.exp(1j * gamma[k] * h) + I[2 * k+1][2, 0] * np.exp(1j * gamma[k] * (thickness[k] - h))
                t += 1
            h = 0
...
class Structure:
    ...
    def __init__(self, materials, layer_type, thickness, verbose=False, unit="nm", si_units=False):
...
def coefficient_S(struct, wavelength, incidence, polarization) :
    ...
    Also, you should not use two nonlocal material structure, it doesn't work for the moment..
    """
    ...
    Kl = [0] * (g - 1)
    omega = [0] * (g - 1)
    Beta = [struct.materials[Type[k]].beta for k in range(g)]
    ...
    for k in range(g - 1) :
        ...
        # DEFINITION DE KL ET OMEGA POUR LE NON LOCALE / MATRICE DE COUCHE NON LOCALE
            ...
            Kl[k] = np.sqrt(alpha**2 + (values_nl(k, wavelength)[2] / Beta[k])**2 * (1 / values_nl(k, wavelength)[1] + 1 / (1 + values_nl(k, wavelength)[0]) ))
            omega[k] = (alpha**2 / Kl[k]) * (1 / Epsilon[k] - 1 / (1 + values_nl(k, wavelength)[0]))
            t = np.exp(1j * gamma[k] * thickness[k])
            l = np.exp(- Kl[k] * thickness[k])
            T.append(np.array([[0, 0, t, 0], [0, 0, 0, l], [t, 0, 0, 0], [0, l, 0, 0]], dtype = complex))
        ...
        if Beta[k] == 0 and Beta[k + 1] == 0 :
            T.append(np.array([[b1 - b2, 2 * b2], [2 * b1, b2 - b1]] / (b1 + b2), dtype = complex))
            #print(f"Matrice {2 * k + 2} d'interface (locale locale) :\n {T[2 * k + 2]}")

        # local non-local interface
        elif Beta[k] == 0 and Beta[k + 1] != 0  :
            Kl[k + 1] = np.sqrt(alpha**2 + (values_nl(k+1, wavelength)[2] / Beta[k+1])**2 * (1 / values_nl(k+1, wavelength)[1] + 1 / (1 + values_nl(k+1, wavelength)[0]) ))
            omega[k+1] = (alpha**2 / Kl[k+1]) * (1 / Epsilon[k+1] - 1 / (1 + values_nl(k+1, wavelength)[0]))

            T.append(np.array([[b1 - b2 + 1j * omega[k + 1], 2 * b2, 2], [2 * b1, b2 - b1 + 1j * omega[k + 1], 2], [2 * 1j * omega[k + 1] * b1, 2 * 1j * omega[k + 1] * b2, b1 + b2 + 1j * omega[k + 1]]] / (b1 + b2 - 1j * omega[k + 1]), dtype = complex))
            #print(f"Matrice {2 * k + 2} d'interface (locale non-locale) :\n {T[2 * k + 2]}")
            #print("DEBUGG", omega[k+1], b1 - b2 + 1j * omega[k + 1])
        
        # non-local local interface
        elif Beta[k] != 0 and Beta[k + 1] == 0  :
            T.append([[b1 - b2 + 1j * omega[k], -2, 2 * b2], [-2 * 1j * omega[k] *  b1, b1 + b2 + 1j * omega[k], -2  * 1j * omega[k] * b2], [2 * b1, -2, b2 - b1 + 1j * omega[k]]] / (b1 + b2 - 1j * omega[k]))
            #print(f"Matrice {2 * k + 2} d'interface (non-locale locale) :\n {T[2 * k + 2]}")
    ...
    # COMBINAISON DES MATRICES DE SCATTERING
...
def dispersion(struct, wavelength, incidence, polarization):
    return 1/np.abs(coefficient(struct, wavelength, incidence, polarization)[0])
...
def complex_map(struct,wavelength,polarization,real_bounds,imag_bounds,n_real,n_imag):
    ...
    for k in range(n_real):
        for l in range(n_imag):
            T[k,l]=1/dispersion(struct,wavelength,np.arcsin(M[k,l]/np.sqrt(struct.polarizability(wavelength)[0][struct.layer_type[0]] * struct.polarizability(wavelength)[1][struct.layer_type[0]] * k_0**2)), polarization)
...
def angular(structure, wavelength, polarization, theta_min, theta_max,
    ...
    Args:
        ...
        theta_min (float): minimum angle of incidence in radian
        theta_max (float): maximum angle of incidence in radian
...
def steepest(start,tol,step_max,struct,wl,pol):
    ...
    dz= 0.01 * delta
    step = 0
    current = dispersion(struct,wl,z,pol)

    while (current > tol) and (step < step_max):

        grad = (dispersion(struct,wl,z+dz,pol) - current + 1j * (dispersion(struct, wl, z + 1j * dz,pol) - current)) / (dz)
        if abs(grad)!=0 :
        ...
        value_new = dispersion(struct,wl,z_new,pol)
...
def follow_guided_modes(struct, wavelength_list, polarization, neff_min, neff_max, format="n", initial_points=40, plot=True):
...
#def  autres fonctions coefficient
...
def electron():
    charge =-1602176565*10**(-19)
    mass = 9.1093837015*10**(-31)
    return mass, charge

def epsAubb_nl(wavelength):
    ...
    # Defined array for multiply each terms of Gamma, omega and sigma by the required coefficient during the function
    
    Iw = np.array([w for i in range(len(Gamma))])
    
    a=np.sqrt(w*(Iw+1j*Gamma))
    
    # Re(a)*a 
    a=a*np.real(a)/np.sqrt(np.real(a)**2)
    ...
    chi_b=sum(1j*np.sqrt(np.pi)*f*omega_p/(2*np.sqrt(2)*a*sigma)*(wofz(x)+wofz(y)))
    chi_f=-omega_p**2*f0/(w*(w+1j*Gamma0))
    w_p=2*np.pi*(1.602176565*10**(-19))/(6.62606957*10**(-25))*omega_p*np.sqrt(f0)
    beta=1.35e6
    return chi_b, chi_f, w_p, beta

def epsAgbb_nl(wavelength):
    w=6.62606957*10**(-25)*299792458/(1.602176565*10**(-19))/wavelength
    f0=0.821
    Gamma0=0.049
    omega_p=9.01
    ...
    f = np.array(f)
    chi_b=sum(1j*np.sqrt(np.pi)*f*omega_p/(2*np.sqrt(2)*a*sigma)*(np.array(wofz(x)) + np.array(wofz(y))))
